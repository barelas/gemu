 ...
 *
 */
public class LinearlyBoundedPerNodeCalcingRScheduler implements Scheduler {
	
	static Log log = LogFactory.getLog(LinearlyBoundedPerNodeCalcingRScheduler.class);
	static final public String NAME = "LINEAR_BOUND_PER_NODE_CALCED_R";
	Grid grid;
	...
	private boolean nodesRequestedWorkloadIsUnderBound(Task task,Node nodeToBeScheduledTo) {
		double bound = calcR(task,nodeToBeScheduledTo) * (grid.getCurrentTime() - task.getNodeOfOrigin().getCreationTime()) + s;
	...
	private double calcR(Task task,Node nodeToBeScheduledTo) {
		HashSet<Node> users = new HashSet<Node>();
		users.add(task.getNodeOfOrigin());
		for (Task assignedTask:nodeToBeScheduledTo.getAssignedTasks()) {
			users.add(assignedTask.getNodeOfOrigin());
		}
		return nodeToBeScheduledTo.getComputationalCapacity() / (double) users.size();
	}
